<script lang="ts">
    ...
    import { createBaseline, createDraftObject, createObject, deleteObject, exportCSV, exportXlsx, readBaselinedModule, readBaselinedObjects, readCurrentObjects, readModuleFromPath, readObjects, restoreObject, updateTemplate } from "$lib/controllers/Module";
    ...
    let repo: Repository | null = $derived(repository());
    ...
    function handleObjectCreation(obj: Object) {
        return createObject(module!.path, obj)
            .then(() => {
                selectedObject = null;
    ...
    function handleObjectDraftCreation(obj: Object) {
        return createDraftObject(module!.path, obj)
            .then((objs) => {
                selectedObject = null;
                objectFormFlag = false;
                loadAllObjects(module!.path);
    ...
    async function handleBaselineCreation(baseline: Baseline) {
        console.log("Creating baseline: " +  $state.snapshot(baseline.version));
    ...
    */
    ...
    function updateState() {
        const savedState = pageState.getPageState(pageKey);
        if (savedState) {
            ({ scrollX, scrollY, selectedObject, editPanelFlag: objectFormFlag, view, showLinksFlag, showRowNumberFlag, readOnlyFlag } = savedState);
    ...
    function saveCurrentState() {
        ...
        const state = {
            ...
            showRowNumberFlag,
            selectedObject,
            editPanelFlag: objectFormFlag,
            showLinksFlag,
            readOnlyFlag,
        };
        pageState.setPageState(pageKey!, state);
    }
    ...
    function contextClick(id: number|string, item: string) {;
        ...
        {/if}
            <Resizable.Pane order={2}>
                ...
                {:then}
                    ...
                    />
                {:catch e}
                    <span>Error getting data of this module...{e}</span>
                {/await}
            </Resizable.Pane>
    </Resizable.PaneGroup>
</div>
