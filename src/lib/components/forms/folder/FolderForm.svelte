			...
			<div class="grid gap-4 py-4 min-h-42">
				...
				<Dialog.Header>
					...
					<div class="grid grid-cols-4 items-center gap-2">
						...
						<Input id="name" placeholder="My Folder Name" bind:value={folder.name}  class="col-span-3" autocomplete="off"/>
					</div>
				{/if}
			</div>
			<Dialog.Footer>
				...
				<Button on:click={handleFolderUpdate} disabled={(folder.name ==="")}>Save Changes</Button>
			</Dialog.Footer>
		{/if}
	</Dialog.Content>
</Dialog.Root>